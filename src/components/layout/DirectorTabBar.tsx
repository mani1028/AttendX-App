...
const DirectorTabBar = ({ state, navigation, isScrollingDown = false }: any) => {
  const insets = useSafeAreaInsets();
  const { tabBarTranslate, isTabBarVisible } = useAuth();
  const sizes = getResponsiveSizes();
  const pulseAnim = useRef(new Animated.Value(1)).current;

  // Combine local scrolling state and global context state
  const hidden = isTabBarVisible === false || isScrollingDown;

  const tabs = [
    { name: 'Home', label: 'Home', icon: Home, routeIndex: 0 },
    { name: 'Branches', label: 'Branches', icon: School, routeIndex: 1 },
    { name: 'AddBranch', label: 'Add Branch', icon: PlusCircle, routeIndex: 2, isCenter: true },
    { name: 'Billing', label: 'Billing', icon: CreditCard, routeIndex: 3 },
    { name: 'Profile', label: 'Profile', icon: User, routeIndex: 4 },
  ];
    ...
    >
      ...
      <View style={[styles.fabContainer, { top: -(sizes.centerButtonSize / 2) - 4 }]}>
        <Animated.View style={{ transform: [{ scale: pulseAnim }] }}>
          ...
          >
            <PlusCircle size={sizes.centerIconSize} color="#fff" />
          </TouchableOpacity>
        </Animated.View>
        <Text style={styles.moreLabel}>Add Branch</Text>
      </View>
    </Animated.View>
  );
};
